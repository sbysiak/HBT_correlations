...
// ##############################################################  
  try {
   ...
   GetFitParameter("Lambda",&pars[0], &dofix[0], &parmin[0], &parmax[0]);
   GetFitParameter("Rout"  ,&pars[1], &dofix[1], &parmin[1], &parmax[1]);
...
int GetFitParameter(TString aKeyword, Double_t *parval, Int_t *isfixed, Double_t *parmin, Double_t *parmax)
{
  *parval = sMainConfig->GetParameter(aKeyword).Atof(); 
  if (sMainConfig->GetParameter(aKeyword+"FitType") == "fixed") {
    *isfixed = 1;
    PRINT_MESSAGE("Fitting parameter: "<<aKeyword<<" = "<<(*parval)<<" fixed.");
    ...
    *isfixed = 2;
    PRINT_MESSAGE("Fitting parameter: "<<aKeyword<<" = "<<(*parval)<<" limited ["<<(*parmin)<<", "<<(*parmax)<<"]");
  } else {
    *isfixed = 0;
    PRINT_MESSAGE("Fitting parameter: "<<aKeyword<<" = "<<(*parval)<<" free.");
  }
}
